...
import {formattedDates, pluralizeNights} from '@/usefuls';
...
// Reactive Data
const hotelData = shallowRef([])
const hotelsArr = JSON.parse(props.hotels);
...
// Computed Properties
const requestedHotelNames = computed(() => {
	return hotelsArr.map(({name}) => name);
});
const requestedHotelLocations = computed(() => {
	return hotelsArr.map(({location}) => location);
});
const requestedBenefits = computed(() => {
	return hotelsArr.map(({benefits}) => benefits);
});
const hotelErids = computed(() => {
	return hotelsArr.map(({erid}) => erid);
});
const hotelLigals = computed(() => {
