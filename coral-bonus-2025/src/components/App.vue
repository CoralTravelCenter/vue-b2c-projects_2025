...
const itemById: ComputedRef<CashbackData | undefined> = computed(() =>
		cashbackData?.find((item) => item.id === currentId),
);
provide("itemById", itemById)

const currentPromoValues: ComputedRef<number[]> = computed(() =>
		itemById.value!.promotions.map((item) => item.value),
);
const cashbackText: ShallowRef<HTMLSpanElement | null> = shallowRef(null)

function calc(value: number, arr: number[]): string {
...
onMounted(() => {
	...
	const off = DL.onEvent('view_item', (data: any) => {
		if (!data) return;
		const price = data?.ecommerce?.value;
		const cashbackValue = calc(
				price,
				currentPromoValues.value
		);
		cashbackText.value.textContent = `Кешбэк до ${cashbackValue}`;
	});
	onUnmounted(() => {
		off();
		DL.destroy();
	...
	<div class="w-full">
		<div class="rounded-[6px] bg-[#FEEFCD] p-2 flex items-center gap-2">
			...
			<div class="flex flex-col">
				<span
						ref="cashbackText"
						id="coral-bonus-cashback-result"
						class="text-[rgba(0,0,0,0.85)] text-sm font-bold leading-5.5"
				>
				</span>
				<span class="text-[rgba(0,0,0,0.85)] text-[12px] leading-4"
				>на карту CoralBonus</span
				>
			</div>
			<div
					id="coral-bonus-cashback-calculator"
