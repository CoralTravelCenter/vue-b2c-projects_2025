window._resortBrands = {
  brands: [
    {
      name: "RIXOS",
      slogan: "Всё включено — всё эксклюзивно",
      settings: {
        dateRange: ["2025-10-15", "2025-11-15"],
        nights: 7
      }
    },
    {
      name: "FAIRMONT HOTELS & RESORTS",
      slogan: "Сделаем особенное возможным",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "VINPEARL HOTELS & RESORTS",
      slogan: "Квинтэссенция Вьетнама",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "GLORIA HOTELS",
      slogan: "Искусство идеального отдыха",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "MAXX ROYAL",
      slogan: "Мир безграничного отдыха",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "VOYAGE HOTELS",
      slogan: "Отпуск, который превзойдёт ваши ожидания",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "LIBERTY HOTELS",
      slogan: "Предвосхищаем и превосходим ожидания",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "PICKALBATROS HOTELS & RESORTS",
      slogan: "Добро пожаловать домой",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "SUNRISE HOTELS & RESORTS",
      slogan: "Незабываемые каникулы",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "TITANIC HOTELS & RESORTS",
      slogan: "Комфорт вашего дома",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Radisson Hotel Group",
      slogan: "Почувствуйте себя знаменитостью",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "MOVENPICK",
      slogan: "Где каждое мгновение создано для того, чтобы им наслаждались",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Mirage Hotel Group",
      slogan: "Откройте для себя уникальный опыт путешествия",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "ROTANA",
      slogan: "Когда детали имеют значение",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "BM Hotels",
      slogan: "Отдых, продуманный до мгновений",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Hmhhotelgroup",
      slogan: "Лакшери отдых у Персидского залива",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Hilton",
      slogan: "Насладитесь знаменитым сервисом Hilton Hotels & Resorts",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "InterContinental Hotels & Resorts",
      slogan: "Отели, где отдых продуман до мгновений",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Barcelo Hotel Group",
      slogan: "Отели для незабываемых впечатлений",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "WYNDHAM",
      slogan: "Исключительный пляжный отдых на Персидском заливе",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Coral group hotels",
      slogan: "Отдых, проверенный временем",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Royal Star",
      slogan: "Релакс-отдых на Красном море",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "MP Hotels",
      slogan: "Где каждая деталь создана для вас",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Jaz Group",
      slogan: "Отдыхайте и ни о чем не думайте",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "THE ANAM",
      slogan: "Уникальный отдых во Вьетнаме",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Marriott",
      slogan: "Отели, которые вдохновляют на отдых",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Best Western Hotels & Resort",
      slogan: "Испытайте гостеприимство мирового уровня ",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "HotelLinkSolution",
      slogan: "Отдыхайте с удовольствием",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "AKKA HOTELS",
      slogan: "Уникальный отдых ждет вас",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "DOBEDAN HOTELS",
      slogan: "Уникальный эксклюзивный",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "TUI",
      slogan: "Бюджетный отдых высокого класса",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "GREEN NATURE HOTELS ",
      slogan: "Отдохните в объятиях природы Мармариса",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "CORAL TRAVEL GROUP",
      slogan: "",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "ARMAS HOTELS",
      slogan: "",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "CRYSTAL HOTELS",
      slogan: "",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "PALOMA HOTELS",
      slogan: "",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    },
    {
      name: "Accor",
      slogan: "",
      settings: {
        dateRange: ["2025-11-17", "2025-11-30"],
        nights: 7
      }
    }
  ],

  countries: [
    // --- ТУРЦИЯ ---
    {
      name: "Турция",
      brands: [{
        name: "RIXOS",
        hotels: [
          "CLUB PRIVE BY RIXOS BELEK",
          "RIXOS DOWNTOWN ANTALYA - THE LAND OF LEGENDS FREE ACCESS",
          "RIXOS PARK BELEK - THE LAND OF LEGENDS FREE ACCESS",
          "RIXOS PREMIUM BELEK - THE LAND OF LEGENDS FREE ACCESS",
          "RIXOS PREMIUM TEKIROVA - THE LAND OF LEGENDS FREE ACCESS"
        ],
        page: 'https://www.coral.ru/chain/rixos/'
      },
        {
          name: "CORAL TRAVEL GROUP",
          hotels: [
            "XANADU RESORT",
            "SEVEN SEAS HOTEL LIFE",
            "SEVEN SEAS PALMERAS BAY",
            "MARVIDA FAMILY ECO (Ex. OTIUM FAMILY ECO CLUB) ",
            "MARVIDA HOTEL HAPPY SUITES"
          ]
        },
        {
          name: "ARMAS HOTELS",
          hotels: [
            "ARMAS BEACH",
            "ARMAS BELLA SUN",
            "ARMAS GREEN FUGLA BEACH",
            "ARMAS GUL BEACH HOTEL",
            "ARMAS KAPLAN PARADISE"
          ]
        },
        {
          name: "CRYSTAL HOTELS",
          hotels: [
            "CRYSTAL ADMIRAL RESORT SUITES&SPA",
            "CRYSTAL AURA BEACH RESORT & SPA",
            "CRYSTAL BOUTIQUE BEACH RESORT",
            "CRYSTAL CENTRO (EX. AMARA CENTRO RESORT)",
            "CRYSTAL CLUB WORLD OF COLOURS RESORT & SPA"
          ]
        },
        {
          name: "PALOMA HOTELS",
          hotels: [
            "PALOMA FINESSE",
            "PALOMA GRIDA",
            "PALOMA MARINA SUITES",
            "PALOMA OCEANA",
            "PALOMA ORENDA"
          ]
        },
        {
          name: "LIBERTY HOTELS",
          hotels: [
            "LIBERTY FABAY ",
            "LIBERTY HOTELS LYKIA ADULT ONLY",
            "LIBERTY HOTELS LYKIA ",
            "LIBERTY SIGNA",
            "SUNDIA BY LIBERTY OLUDENIZ"
          ]
        },
        {
          name: "AKKA HOTELS",
          hotels: ["AKKA ALINDA HOTEL", "AKKA ANTEDON HOTEL", "AKKA CLAROS HOTEL", "AKKA PRIVATE VILLAS"]
        },
        {
          name: "DOBEDAN HOTELS",
          hotels: ["DOBEDAN BEACH RESORT COMFORT", "DOBEDAN EXCLUSIVE HOTEL & SPA", "DOBEDAN WORLD PALACE"]
        },
        {
          name: "TUI",
          hotels: ["TUI MAGIC LIFE BELDIBI", "AKRA FETHIYE THE RESIDENCE TUI BLUE SENSATORI", "AKRA FETHIYE TUI BLUE SENSATORI", "TUI BLUE GRAND AZUR", "TUI BLUE SARIGERME PARK"]
        },
        {
          name: "GREEN NATURE HOTELS ",
          hotels: ["GREEN NATURE DIAMOND", "GREEN NATURE RESORT"]
        }
      ]
    },

    // --- ЕГИПЕТ ---
    {
      name: "Египет",
      brands: [
        {
          name: "RIXOS",
          hotels: [
            "RIXOS PREMIUM MAGAWISH",
            "CLUB PRIVÉ BY RIXOS SHARM EL SHEIKH",
            "RIXOS PREMIUM SEAGATE",
            "RIXOS RADAMIS SHARM El SHEIKH",
            "RIXOS SHARM EL SHEIKH"
          ]
        },
        {
          name: "Coral group hotels",
          hotels: [
            "SEVEN SEAS JOLIE BAY",
            "XANADU MAKADI BAY",
            "XANADU CLUB MAKADI BAY"
          ]
        },
        {
          name: "PICKALBATROS HOTELS & RESORTS",
          hotels: [
            "PICKALBATROS DANA BEACH RESORT",
            "PICKALBATROS JUNGLE AQUA PARK RESORT - NEVERLAND HURGHADA",
            "PICKALBATROS PALACE RESORT",
            "PICKALBATROS WATER VALLEY RESORT - NEVERLAND HURGHADA",
            "PICKALBATROS WHITE BEACH"
          ]
        },
        {
          name: "Jaz Group",
          hotels: [
            "JAZ AQUAVIVA",
            "JAZ MAKADI OASIS RESORT",
            "JAZ MAKADI SARAYA RESORT",
            "JAZ AQUAMARINE",
            "IBEROTEL REDSINA (ex. MAGIC WORLD SHARM)"
          ]
        },
        {
          name: "SUNRISE HOTELS & RESORTS",
          hotels: [
            "SUNRISE GRAND SELECT DIAMOND BEACH RESORT",
            "SUNRISE GRAND SELECT MONTEMARE RESORT",
            "SUNRISE REMAL BEACH",
            "SUNRISE REMAL RESORT",
            "WHITE HILLS"
          ]
        },
        {
          name: "TITANIC HOTELS & RESORTS",
          hotels: [
            "TITANIC BEACH SPA & AQUA PARK",
            "TITANIC PALACE & AQUA PARK BEACH RESORT",
            "TITANIC RESORT & AQUA PARK",
            "TITANIC ROYAL"
          ]
        },
        {
          name: "Accor",
          hotels: [
            "NAAMA BAY PROMENADE BEACH RESORT ( EX MARRIOTT SSH )",
            "NAAMA BAY PROMENADE MOUNTAIN RESORT ( EX MARRIOTT SSH )",
            "NOVOTEL BEACH",
            "NOVOTEL PALM",
            "SWISSOTEL SHARM EL SHEIKH ALL INCLUSIVE COLLECTION"
          ]
        },
        {
          name: "Royal Star",
          hotels: ["EMPIRE BEACH AQUA PARK", "ROYAL STAR BEACH RESORT"]
        },
        {
          name: "MP Hotels",
          hotels: ["LABRANDA CLUB MAKADI", "LABRANDA ROYAL MAKADI", "LEMON & SOUL MAKADI GARDEN"]
        }
      ]
    },

    // --- ОАЭ ---
    {
      name: "ОАЭ",
      brands: [
        {
          name: "RIXOS",
          hotels: [
            "RIXOS AL MAIRID RAS AL KHAIMAH",
            "RIXOS BAB AL BAHR",
            "RIXOS PREMIUM DUBAI",
            "CLUB PRIVE BY RIXOS SAADIYAT ISLAND",
            "RIXOS MARINA ABU DHABI",
            "RIXOS THE PALM DUBAI HOTEL & SUITES",
            "RIXOS PREMIUM SAADIYAT ISLAND"
          ]
        },
        {
          name: "Marriott",
          hotels: [
            "AJMAN SARAY A LUXURY COLLECTION RESORT",
            "FOUR POINTS BY SHERATON SHARJAH",
            "LE MERIDIEN AL AQAH",
            "SHERATON SHARJAH BEACH RESORT & SPA"
          ]
        },
        {
          name: "WYNDHAM",
          hotels: [
            "RAMADA BEACH HOTEL AJMAN",
            "RAMADA HOTEL & SUITES AJMAN",
            "TRYP BY WYNDHAM",
            "WYNDHAM GARDEN AJMAN CORNICHE"
          ]
        },
        {
          name: "InterContinental Hotels & Resorts",
          hotels: [
            "INTERCONTINENTAL FUJAIRAH RESORT",
            "INTERCONTINENTAL RAS AL KHAIMAH MINA AL ARAB RESORT & SPA"
          ]
        },
        {
          name: "Hilton",
          hotels: [
            "HAMPTON BY HILTON MARJAN ISLAND",
            "DOUBLETREE BY HILTON RESORT & SPA MARJAN ISLAND"
          ]
        },
        {
          name: "Hmhhotelgroup",
          hotels: ["BAHI AJMAN PALACE HOTEL", "CORAL BEACH RESORT SHARJAH"]
        },
        {
          name: "Barcelo Hotel Group",
          hotels: ["OCCIDENTAL SHARJAH GRAND"]
        }
      ]
    },

    // --- ВЬЕТНАМ ---
    {
      name: "Вьетнам",
      brands: [
        {
          name: "VINPEARL HOTELS & RESORTS",
          hotels: ["VINPEARL BEACHFRONT NHA TRANG", "VINPEARL RESORT & SPA NHA TRANG BAY", "VINPEARL RESORT NHA TRANG"]
        },
        {
          name: "THE ANAM",
          hotels: ["THE ANAM MUI NE"]
        },
        {
          name: "Marriott",
          hotels: ["NHA TRANG MARRIOTT RESORT & SPA, HON TRE ISLAND"]
        },
        {
          name: "Best Western Hotels & Resort",
          hotels: ["BEST WESTERN PREMIER MARVELLA NHA TRANG HOTEL"]
        },
        {
          name: "HotelLinkSolution",
          hotels: ["ALMA RESORT CAM RANH"]
        }
      ]
    }
  ]
};
